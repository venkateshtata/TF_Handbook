...
def deconv(x, w, b, shape, stride, name):
	with tf.variable_scope('deconv'):
		tf.summary.histogram('weight', w)
		tf.summary.histogram('bisases', b)
...
def train(logdir, batch_size):
	...
	with tf.variable_scope('placeholder'):
		...
		with tf.variable_scope('D_loss'):
			...
			d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_fake_logits, labels=tf.ones_like(D_real_logits)))
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(description='Train vanilla GAN using convolutional networks')

	parser.add_argument('--logdir', type=str, default='1', help='logdir for tensorboard')
